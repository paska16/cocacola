    ...
    if(count($profile_links)>0){
        ...
        <ul class="smartlib-author-profile-links list-inline">
            ...
            foreach($profile_links as $key =>$row){
               if(strlen($row)>0){
                ?>
                <li><a href="<?php echo $row ?>" class="smartlib-icon smartlib-small-crcle-icon smartlib-<?php echo $key ?>-ico"><i class="<?php echo apply_filters('smartlib_get_awesome_ico', 'fa fa-share', $key); ?>"></i></a></li>
            <?php
               }
            }
            ?>
...
/**
 * Wrapping header attrubutes filter
 * @param string $default
 */

function smartlib_header_attributes($default='role="navigation" '){

    echo apply_filters('smartlib_header_attributes', $default);

}

/**
 * Wrapping header classes filter
 * @param string $default
 */
function smartlib_header_wrapper_classes($default='smartlib-main-navigation-wrapper'){

    echo apply_filters('smartlib_header_wrapper_classes', $default);

}

function smartlib_layout_class( $default_class = '', $type = 'default' ){

        echo apply_filters('smartlib_content_layout_class', $default_class, $type);

}

function smartlib_get_context_sidebar($type = 'default'){

    return apply_filters('smartlib_get_context_sidebar', $type);

}

function smartlib_get_sidebar_template($template, $type = 'default'){

    return apply_filters('smartlib_get_sidebar_template', $template, $type);

}

/**
 * Final settings wrapper
 * @param $global
 * @param $specific
...
function smartlib_get_final_settings($global, $specific, $default_value='', $setting_type=''){

    return __SMARTLIB_HELPERS::smartlib_get_final_settings($global, $specific, $default_value,  $setting_type);
...
function smartlib_get_settings_based_on_type($setting_prefix, $type, $default=0){

    return __SMARTLIB_HELPERS::smartlib_get_settings_based_on_type($setting_prefix, $type, $default);
...
function smartlib_top_bar(){

    do_action('smartlib_top_bar');
}
