...
if (function_exists('siteorigin_panels_init') || function_exists('siteorigin_widget_get_plugin_path')) {
    ...
    add_filter('siteorigin_panels_css_row_gutter', 'smartlib_change_gutter', 10, 1);
    //add_filter('siteorigin_panels_css_object', 'smartlib_panels_css_object', 10, 3);

    add_filter('siteorigin_panels_css_object', 'smartlib_panels_css_object', 10, 4);


    /*
    $css->add_row_css( $post_id, $ri, array( '.panel-has-style > .panel-row-style' ), array(
...
function smartlib_custom_page_builder_css()
{
    wp_enqueue_style(
        'smartlib_main'
    ...
    wp_add_inline_style('smartlib_main', $custom_css);
}


add_action('wp_enqueue_scripts', 'smartlib_custom_page_builder_css');

function smartlib_panels_css_object($css, $panels_data, $post_id, $layout_data)
{


    $css->add_row_css(false, false, false, array(
...
function smartlib_row_style_fields($fields)
{
    ...
    */
    ...
    $fields['background_image_attachment_smartlib'] = array(
        'name' => __('Background Image', 'bootframe-core'),
        'type' => 'image',
        'group' => 'design',
        'description' => __('Background image of the row', 'bootframe-core'),
        'priority' => 6,
    );
...
function smartlib_add_html_before_row($panels_data_grid, $grid_attributes)
{
    ...
    /**
     * Remove values because used on lower level
     * (includes paddig in full strech  sections
     */

    if (isset($grid_attributes['style']['row_stretch']) && $grid_attributes['style']['row_stretch'] != 'full-stretched') {
        if (isset($grid_args['padding']))
            unset($grid_args['padding']);
    }

    if (isset($grid_args['border_color']))
        unset($grid_args['border_color']);
    ...
    if (isset($grid_attributes['style']['bottom_margin'])) {

        $style_substring .= 'margin-bottom:' . $grid_attributes['style']['bottom_margin'] . ';';

    }

    //new version Site Origin

    if (isset($grid_args['background'])) {
        $style_substring .= 'background-color:' . $grid_args['background'] . ';';
    }

    if (isset($grid_args['padding'])) {
        $style_substring .= 'padding:' . $grid_args['padding'] . ';';
    }

    if (isset($grid_args['background_image_attachment_smartlib'])) {

        $style_substring .= 'background-image: url(' . wp_get_attachment_url($grid_args['background_image_attachment_smartlib']) . ');';

    }

    if (isset($grid_args['background_display'])) {
        $style_substring .= 'background-size:' . $grid_args['background_display'] . ';';
    }


    $style_string = isset($style['style']) ? ' style="' . $style['style'] . $style_substring . '"' : '';
    ...
    $section_attributes .= isset($grid_args['overlay_background_color']) && !empty($grid_args['overlay_background_color']) ? ' data-type="background"' : '';
    ...
    if (isset($grid_attributes['style']['row_stretch'])) {
        ...
        if ($grid_attributes['style']['row_stretch'] == 'full-stretched') {
            ...
            echo '<div class="smartlib-blck-area ' . $section_class . '" ' . $section_attributes . $style_string . '><div class="container">';
        } elseif ($grid_attributes['style']['row_stretch'] == 'full') {
            echo '<div class="smartlib-blck-area ' . $section_class . '" ' . $section_attributes . '><div class="container-fluid"' . $style_string . '>';
        } else {
            echo '<div class="smartlib-blck-area"><div class="container-fluid ' . $section_class . '"' . $style_string . $section_attributes . '>';
        }
    } else {

        echo '<div class="smartlib-blck-area"><div class="container">';
    }
}
...
/**
 * Cahnge default page builder gutter
 * @return mixed
 */

function smartlib_change_gutter()
{

    return Smartlib_Config::getInstance()->project_default_sizes['project_gutter_width'] . 'px';
